...
class SecretionCellSteppable(SteppableBasePy):
    def __init__(self,_simulator,_frequency=1):
        ...
        #self.fieldNameProtease = 'Protease'
    def start(self):
...
class VolumeParamSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        for cell in self.cellList:
            if cell.type==1:   # endothelial stalk cells (ECs)
                cell.targetVolume=30
                cell.lambdaVolume=6.0
                cell.targetSurface=4*sqrt(cell.targetVolume)
                cell.lambdaSurface=4.0
            if cell.type==2:   # macrophage/inflammatory cells (IC)
                cell.targetVolume=40
                cell.lambdaVolume=6.0
                cell.targetSurface=4*sqrt(cell.targetVolume)
                cell.lambdaSurface=4.0
            if cell.type==3:    # mural/VSMC cells (MC)
                cell.targetVolume=50
                cell.lambdaVolume=6.0
                cell.targetSurface=4*sqrt(cell.targetVolume)
                cell.lambdaSurface=4.0
            if cell.type==4:   # endothelial tip cells (ECt)
                cell.targetVolume=30
                cell.lambdaVolume=6.0
    ...
    def step(self,mcs):
        ...
        ECapoptosisratevariable=10000 #EC SRB down 
          ...
          #Check to ensure cell neighbor is not medium or other cell types
            ...
            #sum up common surface area of cell with its neighbors
                ...
                if  randint(1,ECapoptosisratevariable)==1:
                    cell.type=6
                    Apoptotic = 1
                if totalArea<26 and VEGFconcentration>0.8:      #VEGFR2 inhibition - takes twice as much VEGF to instigate growth (control:0.4)               
                    if CXCL10concentration>0.3:
                        if CXCL10concentration>0.6:
                            ...
                            if VEGFconcentration>1:
                                ...
                                cell.targetVolume+=0.1 #EC proliferation, control is 0.8
                                cell.targetSurface=4*sqrt(cell.targetVolume)
		...
		randommultiplier=uniform(0.8,1.2)
                # if mcs>2000: # and mcs<12000:
		cell.targetVolume+=(0.001*randommultiplier)   #Inflammatory Cell Proliferation control=0.02
		cell.targetSurface=4*sqrt(cell.targetVolume)
            ...
            if cell.type==3:
                randommultiplier=uniform(0.8,1.2)
                #if mcs>2000:
		cell.targetVolume+=(0.005*randommultiplier)  ##Mural Cell Proliferation control=0.02
		cell.targetSurface=4*sqrt(cell.targetVolume)           
                # cell.targetVolume+=(0.05*randommultiplier)
                totalECArea = 0
                pt=CompuCell.Point3D()
                pt.x=int(round(cell.xCM/max(float(cell.volume),0.001)))
                pt.y=int(round(cell.yCM/max(float(cell.volume),0.001)))
          ...
          # #Check to ensure cell neighbor is not medium or other cell types
            ...
            #sum up common surface area of cell with its neighbors
                ...
                # print "Total VEGF concentration:",VEGFconcentration
                    ...
                    if totalECArea>5:                                  
                        cell.targetVolume+=(0.01*randommultiplier)  #Mural Cell Proliferation control=0.02
                        cell.targetSurface=4*sqrt(cell.targetVolume)
